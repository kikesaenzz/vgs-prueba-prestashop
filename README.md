...
- **La pestaña "Detalles del producto" se retira entera.** Falcon la añade por defecto debajo de "Información del producto", pero no está en el diseño y repetía datos que ya se muestran más arriba (marca/referencia en la banda de meta, características en su tabla). Se quita la inclusión completa en vez de vaciar el bloque, porque no queda ningún tab que sí forme parte del diseño.
- **Los subtítulos de "Información del producto" van sin marca de verificación delante.** El diseño original usaba un "✓" como viñeta antes de cada apartado ("Características destacadas...", "Usos...", "Modo de aplicación...", "Presentación y formato"); se ha retirado a petición expresa, dejando solo el título en color.
...
- **El hueco del selector de ordenación pasa a mostrar el filtro activo.** El diseño no lleva "Relevancia" ni el resto de opciones que Falcon pone ahí por defecto; en ese mismo sitio se pinta ahora el aviso de filtros activos, que Falcon pintaba aparte y solo en escritorio. El HTML de `ps_facetedsearch` lo envuelve en una tarjeta de Bootstrap pensada para ir en su propia caja; el CSS anula ese marco para que quede en línea, como en el Figma.
...
**24. Concatenar una query string a pelo no comprueba si ya lleva "?".**
El icono de cuenta generaba `...?controller=authentication?back=...`, con dos interrogantes: `$urls.pages.authentication` ya trae `?controller=...` en esta tienda (sin URLs amigables), y el `href` le concatenaba un segundo `?back=` en vez de `&back=`. El navegador no lo interpretaba como dos parámetros y el "volver aquí" tras iniciar sesión no funcionaba. Arreglado comprobando con `strpos` si la URL base ya lleva un `?` antes de decidir el separador, en vez de asumir cuál hace falta.

**25. Estilar el contenedor exterior no basta si el problema está en las clases genéricas de dentro.**
"Filtros activos" debía ir en una sola línea junto a las píldoras, sin ninguna caja alrededor, y mi primer intento de CSS no lo conseguía. El motivo: `ps_facetedsearch` envuelve el título y las píldoras en una tarjeta de Bootstrap (`.card > .card-header + .card-body`, pensada para vivir en su propia caja aparte), y mi regla solo tocaba el `<section>` exterior — que tiene un único hijo, así que ponerle `display: flex` no cambiaba nada del layout *interno* del `.card`. Hubo que anular explícitamente el marco de la tarjeta y poner `.card-header`/`.card-body` en línea con flexbox, no solo el contenedor de más afuera.

**26. Que algo "funcione" no significa que sea visible en todos los tamaños.**
El mismo aviso de filtros activos llevaba, heredada del bloque original, una clase que Falcon usa para ocultar contenido por debajo de 768px (pensada para cuando ese aviso vivía en otro sitio y el móvil usaba el modal "Filtrar" en su lugar). Al moverlo de sitio no se revisó si esa ocultación seguía teniendo sentido: en móvil no aparecía nunca, con o sin filtro activo, aunque el código que lo genera funcionaba perfectamente. Comprobar "¿aparece en escritorio?" no basta para dar un elemento por bueno cuando la prueba pide responsive.
...
- Registro de cuenta y login probados de extremo a extremo (cuenta real creada y borrada tras la prueba): quedan en español, y el "volver aquí" tras iniciar sesión funciona.
- El aviso de filtros activos comprobado en 375 px y en escritorio, con y sin filtro marcado, y entrando por cada una de las siete subcategorías.
...
- **Cuatro subcategorías (Capilar, Ojos, Labios, Fragancias) se quedaron sin ningún producto** tras crear el catálogo. Sin productos posibles, `ps_facetedsearch` ni siquiera ofrece esa subcategoría como filtro marcable, y navegar a ella desde el sidebar llevaba a un listado vacío — el ejemplo del propio Figma, "Categorías: capilar", no se podía reproducir por este motivo. `vgs-completar-subcategorias.php` asocia a cada una un producto ya existente (categoría secundaria, sin tocar la principal), sin inventar productos ni nombres nuevos.
